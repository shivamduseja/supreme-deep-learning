...
## adding configuration for resnet 
parser.add_argument('--resnet', type=int, default = 1, help='use pre-trained model')
parser.add_argument('--type' , choices= ['softmax', 'convnet', 'twolayernn', 'mymodel'], help='resnet type')
parser.add_argument('--part' , type = int , default = 0 , help='resnet type')
...
if args.model == 'softmax':
    ...
    model_1 = models.softmax.Softmax(im_size, n_classes)
elif args.model == 'twolayernn':
    # Problem 3 (one hidden layer, input -> hidden layer -> output)
    model_1 = models.twolayernn.TwoLayerNN(im_size, args.hidden_dim, n_classes)
elif args.model == 'convnet':
    # Problem 4 (multiple hidden layers, input -> hidden layers -> output)
    model_1 = models.convnet.CNN(im_size, args.hidden_dim, args.kernel_size, n_classes)
elif args.model == 'mymodel':
    # Problem 5 (multiple hidden layers, input -> hidden layers -> output)
    model_1 = models.mymodel.MyModel(im_size, args.hidden_dim, args.kernel_size, n_classes)
else:
...
# if resnet defining resnet model
if args.resnet == 0:
    model = models.resnet.Resnet(args.type, args.part, model_1)
else:
    model = model_1
...
#############################################################################
if args.resnet == 0:
    #params = model.res_layer.fc.parameters()
    params = model.res_layer.parameters()
else:
...
def train(epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        loss = criterion(target_pred, targets) #reduction='sum'
        optimizer.zero_grad()
...
if args.resnet == 0:
    torch.save(model, args.model + str(args.part) + 'resnet.pt')
else:
