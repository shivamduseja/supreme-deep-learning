...
# Looking at correct and misclassified labels
correctlabel=list()
misclassifiedlabel=list()

## Load the proper neural network model.
if args.model == 'softmax':
    # Problem 2 (no hidden layer, input -> output)
    model = softmax.Softmax(im_size, n_classes)
    
elif args.model == 'twolayernn':
    # Problem 3 (one hidden layer, input -> hidden layer -> output)
    model = twolayernn.TwoLayerNN(im_size, args.hidden_dim, n_classes)
    
elif args.model == 'convnet':
    # Problem 4 (multiple hidden layers, input -> hidden layers -> output)
    model = convnet.CNN(im_size, args.hidden_dim, args.kernel_size, n_classes)
    print('model params')
    print('--------------------------------------------------')
    print('kernel size-',args.kernel_size,'  hidden dimen-',args.hidden_dim,'  epochs-',args.epochs,'  weight decay',args.weight_decay,'  momentum', args.momentum,' learning rate',args.lr,'  batch size',args.batch_size)
    print('--------------------------------------------------')
    
elif args.model == 'mymodel':
    # Problem 5 (multiple hidden layers, input -> hidden layers -> output)
    model = mymodel.MyModel(im_size, args.hidden_dim, args.kernel_size, n_classes)
    print('--------------------------------------------------')
    print('kernel size-',args.kernel_size,'  hidden dimen-',args.hidden_dim,'  epochs-',args.epochs,'  weight decay',args.weight_decay,'  momentum', args.momentum,' learning rate',args.lr,'  batch size',args.batch_size)
    print('--------------------------------------------------')
...
def evaluate(split, verbose=False, n_batches=None):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(loader):        
            ...
            #plt.imshow(data[2],data[3],data[1])
...
print('misclassifiedlabel-------->>>>>>>>>',misclassifiedlabel)
#def imshow(img):
    ...
    #print(npimg)
    #print(npimg.shape)
    #norm = simple_norm(npimg, 'sqrt')
    #plt.imshow(np.transpose(npimg, (1, 2, 0)),norm=norm)
    #plt.show()
    
#dataiter = iter(val_loader)
#images, labels = dataiter.next()
#for btch,img in enumerate(images):
